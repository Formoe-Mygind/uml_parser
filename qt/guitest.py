...
class MainWindow(QDialog):
    ...
    def display_state(self):
        ...
        try:
            main_parse(self.state_path, self.sequence_path)
        except Exception:
            self.status.setText("File not found")
        self.w.append(StateDisplay())

        self.w[-1].show()
        self.status.setText("Done!")


class StateDisplay(QDialog):

    def __init__(self):
        super(StateDisplay, self).__init__()
        PyQt5.uic.loadUi('state_display.ui', self)
        self.photo_state.setPixmap(QtGui.QPixmap("pim_state.png"))
        self.photo_seq.setPixmap(QtGui.QPixmap("pim_sequence.png"))
...
from v2 import sequence_tracer as sequence, state_tracer
...
if __name__ == '__main__':
    ...
    def main_parse(state_filepath, sequence_filepath):

        #print(MainWindow.browse_state("x"))
        #print(MainWindow.browse_seq("x"))
        # FILEPATHS ARE HARDCODED FOR DEMO PURPOSES.
        # state_filepath = 'example_state_machine'
        #state_filepath = f'{MainWindow.browse_state("x")}'
        # sequence_filepath = 'example_sequence_diagram'
        #sequence_filepath = f'{MainWindow.browse_seq("x")}'
